...
#python3 extract-features.py "$BASEDIR/data/devel/" "$BASEDIR/resources/HSDB.txt" > "$TRAIN_DIR/devel.cod" &
...
#python3 "$PREDICT_DIR/predict-sklearn.py" "$MODELS_DIR/model_nb.joblib" "$PREDICT_DIR/vectorizer.joblib" < "$TRAIN_DIR/devel.cod" > "$OUTPUT_DIR/devel-NB.out"
## evaluate Naive Bayes results
#echo "Evaluating Naive Bayes results..."
#python3 evaluator.py DDI "$BASEDIR/data/devel" "$OUTPUT_DIR/devel-NB.out" > "$OUTPUT_DIR/devel-NB.stats"
#
...
#python3 "$PREDICT_DIR/predict-xgb.py" "$MODELS_DIR/model_xgb.joblib" "$PREDICT_DIR/vectorizer.joblib" "$PREDICT_DIR/label_encoder_xgb.joblib" < "$TRAIN_DIR/devel.cod" > "$OUTPUT_DIR/devel-XGB.out"
## evaluate XGBoost results
#echo "Evaluating XGBoost results..."
#python3 evaluator.py DDI "$BASEDIR/data/devel" "$OUTPUT_DIR/devel-XGB.out" > "$OUTPUT_DIR/devel-XGB.stats"
...
python3 "$PREDICT_DIR/predict-sklearn.py" "$MODELS_DIR/model_svc.joblib" "$PREDICT_DIR/vectorizer.joblib" < "$TRAIN_DIR/devel.cod" > "$OUTPUT_DIR/devel-SVC.out"
# evaluate SVC results
echo "Evaluating SVC results..."
python3 evaluator.py DDI "$BASEDIR/data/devel" "$OUTPUT_DIR/devel-SVC.out" > "$OUTPUT_DIR/devel-SVC.stats"
